...
if __name__ == "__main__":
    ...
    while True:
        alarm_on = False  # Flag to track if the alarm is currently playing
        ret, frame = cap.read()
        if not ret:
            break
        ...
        processed_frame, play_alarm = handler.process(rgb_frame, thresholds)

        processed_frame = detect_seatbelt(processed_frame)
        
        if play_alarm and not alarm_on:
            #print("Playing alarm...")
            alarm_on = True
